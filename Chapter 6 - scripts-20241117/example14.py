...
tempC = pd.read_excel(file_path, sheet_name='Sheet1', header=None).iloc[1, 1:]  # Adjusts automatically to existing data
...
with pd.ExcelWriter(file_path, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
    ...
    output_data.to_excel(writer, sheet_name='Sheet1', startcol=1, startrow=2, index=False, header=False)
